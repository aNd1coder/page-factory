...
$(function () {
  ...
  function init() {
    ...
    $(config.canvas).on('contextmenu dblclick', 'a', function (e) {
      ...
      $(config.htag).val($cur.data('htag'));

      if ($cur.data('coupon')) { // 优惠券
        $(config.link).val($cur.data('coupon'));
      } else if ($cur.data('spu-id')) { // H5商详
        $(config.link).val($cur.data('spu-id'));
        $('#link_tit').val($cur.data('sku-id'));
      } else { // 普通链接
        var link = $cur.attr('href').replace(/\?htag=.*/g, '');
        var htag = $cur.attr('href').toLocaleLowerCase().indexOf('htag') > 0 ? $cur.attr('href').match(/htag=([.0-9]*)/i)[1] : '';
        $(config.link).val(link);
        $(config.htag).val(htag);
      }
    ...
    $('#btn_confirm').click(function () {
      var type = $(config.link_type).val(),
          link = $.trim($(config.link).val()),
          title = $.trim($('#link_tit').val()),
          htag = $.trim($(config.htag).val()),
          target = $('#open_type').find('input:checked')[0].id;
      ...
      if ($cur) {
        if (type == '') { // 普通链接
          ...
          if (link.toLocaleLowerCase().indexOf('htag') < 0) {
            // 本来没有，新增 htag
            if (htag != '') {
              var hash = link.indexOf('#') != -1 ? link.replace(/.*?(#.*)/ig, '$1') : '';
              $cur.attr('href', link.replace(/#.*/ig, '') + '?htag=' + htag + hash);
            } else {
              tip('htag必须填写');
              return;
            }
          } else {
            $cur.attr('href', htag != '' ? link.replace(/htag=([.0-9]*)/ig, 'htag=' + htag) : link);
          }

          $cur.attr({title: title, target: target});
          ...
          $cur.attr({href: 'javascript:;', title: title, 'data-coupon': link, 'data-htag': htag});
        } else if (type == 'item') { // H5商详
          if ('' == link) {
            tip('请填写SPUID');
            return;
          }

          $cur.attr({href: 'javascript:;', 'data-spu-id': link, 'data-sku-id': title, 'data-htag': htag});
        }
  ...
  function getCode() {
    ...
    return $(config.workshop).clone()
        ...
        .find(config.hot_area).css('position', '').removeAttr('class').each(function (idx) {
          $(this).addClass('hota-item hota-item-' + (idx + 1));
        }).end().html();
  }
